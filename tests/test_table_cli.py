...
def _load_table_cli_module():
    spec = importlib.util.spec_from_file_location("build_comparison_table_cli", SCRIPT_PATH)
...
def _sample_dataframe_with_time_values_that_differ_from_timestamp_deltas() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "Status": ["Rest", "CC_DChg", "CC_DChg", "CC_DChg"],
            "Timestamp": [
...
def test_cli_defaults_labels_uses_default_output_and_warns_for_missing_extrema(
    ...
    def fake_default_table_output_path(*, resolved_y_column: str, anchor_x: float) -> Path:
    ...
    def fake_save_comparison_table(
        *, rows, anchor_x: float, output_path: Path, extrema_header_labels=None
        ...
        output_path.write_text("name;10;;;;;\n;+U_l;+U_m;+U_r;-U_l;-U_m;-U_r\n", encoding="utf-8")
        return output_path

    monkeypatch.setattr(module, "load_ndax_dataframe", fake_load_ndax_dataframe)
    monkeypatch.setattr(module, "default_table_output_path", fake_default_table_output_path)
    monkeypatch.setattr(module, "find_six_extrema_indices", fake_find_six_extrema_indices)
    monkeypatch.setattr(module, "save_comparison_table", fake_save_comparison_table)
...
def test_cli_uses_public_y_label_units_for_time_extrema_headers(monkeypatch, tmp_path: Path) -> None:
    module = _load_table_cli_module()
    captured: dict[str, object] = {}
    ...
    assert captured["y_series"] == pytest.approx([0.0, 4.0 / 3600.0, 9.0 / 3600.0])
    assert captured["extrema_header_labels"] == (
        "+U_l(h)",
        "+U_m(h)",
        "+U_r(h)",
...
def test_cli_uses_non_time_x_column_as_is_without_cumulative_transform(
    ...
    def fail_if_called(**_kwargs):
        raise AssertionError("default_table_output_path should not be called when --output is provided")
