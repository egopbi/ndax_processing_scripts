...
def test_trim_leading_rest_rows_removes_only_initial_contiguous_block() -> (
    None
):
...
def test_prepare_x_series_uses_non_time_columns_as_is() -> None:
    ...
    assert prepare_x_series(dataframe, "Current(mA)").equals(
        dataframe["Current(mA)"]
    )
...
def test_prepare_x_series_for_real_ndax_time_is_monotonic_and_starts_at_zero() -> (
    None
):
