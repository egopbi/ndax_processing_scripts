...
def _available_columns(columns_source: pd.DataFrame | Iterable[str]) -> list[str]:
...
def resolve_column_name(columns_source: pd.DataFrame | Iterable[str], user_column_name: str) -> str:
    ...
    for available_column in available_columns:
        normalized_to_columns[normalize_column_name(available_column)].append(available_column)
    ...
    if len(matches) > 1:
        ...
        raise ValueError(f"Ambiguous column '{user_column_name}'. Matching columns: {matched}")
...
def resolve_column_names(
    ...
    return [resolve_column_name(columns_source, user_column_name) for user_column_name in user_column_names]
