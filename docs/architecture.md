...
5. validates output image width and height in pixels before export
6. writes either one combined image or one image per input file through the export helpers
...
- `table_data_extraction.plotting`: plot column resolution, axis labels, figure size validation, and figure export
